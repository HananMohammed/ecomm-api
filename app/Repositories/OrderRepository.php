        ...
        try {
            if ($request->payment_gateway == 'strip'){
                ...
                if (!empty($wallet)){
                     ...
                     if ($wallet[0]['money'] >= $totalAmount ){
                         $order = $this->addToOrdersTables($request, null, $product);
                         $balance = [ "Balance" => $wallet[0]['money'] - $totalAmount ];
                         Wallet::find($wallet[0]['id'])->update([
                             'money' => $wallet[0]['money'] - $totalAmount
                         ]);
                         return $this->returnData('data', $balance, __('messages.order_success'));

                     }else{
                         return $this->returnError(403,__('messages.money_less'));
                     }
                }else{
                    return $this->returnError(403,__('messages.wallet_not_created'));
                }
            }else{
                return $this->returnError(500,__('messages.payment_not_support'));
            }
